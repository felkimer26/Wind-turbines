...
public static void main(String[] args) {
 ...
 while (!input.equals("Q")) {
            ...
            if (input.equals("1")) {
                ...
                for(String[] p: windData){
                    String name = p[0];
                    name = name.replace("\"", "");
                    if(!statelist.contains(name)){statelist.add(name);
                    }
                }
                System.out.println(statelist);
                String maxstate = " ";
                int maxnum = 0;
                for(String q: statelist){
                    int pnum = 0;
                    for (String[] p : windData.subList(1, windData.size()) ){
                        String qp = p[0].replace("\"", "");
                        
                    if(qp.equals(q)){
                         String numTurbines = p[9];
                        numTurbines = numTurbines.replace("\"", "");
                         int value = Integer.parseInt(numTurbines);
                         pnum+= value;
                        
                    }}
                   if (pnum> maxnum){maxnum = pnum; maxstate = q;} 
                }
                System.out.println("The state with the most wind turbines is: "+ maxstate + " with: "+ maxnum + " wind turbines.");
            } 
